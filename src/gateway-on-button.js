...
function onclickStatus(gateway) {
  ...
  ////////random indicator////////
  var indicator =
    gateway.indicators[Math.floor(Math.random() * gateway.indicators.length)];
  ...
  gateway.publisher.data = {
    ...
    body: {
      ...
      id:
        hwgb > 0.5
          ? gateway.model.id
          : gateway.indicators[Math.floor(hwgb * gateway.indicators.length * 2)]
              .model.id,
      message: ['ERR001', 'ERR002', 'ERR003', 'ERR004', 'ERR005', 'ERR006'][
